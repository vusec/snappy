// RUN: %clang_dfsan_snapshot -O3 -g %s -o %t
...
__attribute__((always_inline)) char caller_1(const char *buffer) {
  return buffer[0];
}

__attribute__((always_inline)) char caller_2(const char *buffer) {
