...
#[no_mangle]
pub unsafe extern "C" fn __dfsw___angora_trace_switch_tt(
    cmpid: u32,
    context: u32,
    size: u32,
    condition: u64,
    ...
    let sw_args = slice::from_raw_parts(args, num as usize);
...
#[no_mangle]
pub unsafe extern "C" fn __dfsw___angora_trace_fn_tt(
    cmpid: u32,
    context: u32,
    size: u32,
    parg1: *mut i8,
    ...
    log::trace!("[FN] id: {}, ctx: {}, size: {}", cmpid, context, size);

    let (arglen1, arglen2) = if size == 0 {
        (libc::strlen(parg1) as usize, libc::strlen(parg2) as usize)
    } else {
        (size as usize, size as usize)
    };

    let lb1 = dfsan_read_label(parg1, arglen1);
    let lb2 = dfsan_read_label(parg2, arglen2);
