use crate::dfsan::{dfsan_create_label, dfsan_label, dfsan_set_label};
...
impl Tainter {
    ...
    pub fn trace_read(
        ...
        let relevant_tainted_offsets_start = begin_range_pos_res.unwrap_or_else(|offset| offset);
        ...
        for read_offset in 0..read_size {
            if let Some(next_relevant_tainted_offset) = relevant_tainted_offsets_iter.peek() {
                ...
                if **next_relevant_tainted_offset == current_file_offset {
                    ...
                    let offset_label = self.label_manager.get_or_create_label(current_file_offset);
